...
class EwayBill extends GSTCommon
 {
	...
	*/
	public function generateToken($new_sales_id) {
		...
		$this->echoToTriage("<br><u><b>Eway Bill Oauth Request</b></u>");
	...
	*/  
    public function authentication($new_sales_id) {
		...
		if($isProd==true){
			$authURL='https://gsthero.com/ewb/enc/v1.03/authentication';
		}else{
			$authURL='https://qa.gsthero.com/ewb/enc/v1.03/authentication';
		}
		...
		if(isset($sessionAuth)){
		    $this->echoToTriage("<br><b><u> Session Eway Bill Authentication token is present so get this token </b></u> <br>Details: ");
		...
		$this->echoToTriage("<br><br><b><u>Eway Bill Authentication Request</u></b><br> Oauth Token for authentication: ".$token);
        if(isset($token)){
		  ...
		  //curl_setopt($ch, CURLOPT_URL,'https://qa.gsthero.com/ewb/enc/v1.03/authentication');
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
          curl_setopt($ch, CURLOPT_POST, true);
          ...
          if ($out === false) {
              $this->echoToTriage('<br>Eway Bill Authentication : Curl error : ' . curl_error($ch));
           }
		  curl_close($ch); 
		  
		  $authToken = $res['access_token'];
		 $this->echoToTriage("<br>Response for generate Eway Bill Authentication token: " . $authToken);
		 //Get the token and then set to session for further usage
		  if(isset($authToken)){
				$_SESSION["auth-Token1"] = $authToken;
				$this->echoToTriage("<br>Session Eway Bill Authentication is set now: " . $authToken); 
	...
	*/
    function execute($url,$data,$action,$Authorization,$XConnectorAuthToken) {
       ...
       //$this->echoToTriage("<br> URL:  ".$url);
       $this->echoToTriage("<br> this->getBaseClientGSTNo(): ".$this->getBaseClientGSTNo());
       $this->echoToTriage("<br> action: ".$action);
       //$this->echoToTriage("<br> Authorization: ".$Authorization);
       //$this->echoToTriage("<br> XConnectorAuthToken: ".$XConnectorAuthToken);
